...
mainContent.append(newDivTag);
   ...
   function displayLogin()
    ...
    function testEmail()
    {
      ...
      let emailPattern = /([A-Z][a-z]{1,25})+(\s|,|-)([A-Z][a-z]{1,25})+(\s|,|-)*/;
        ...
        $("#emailAddress").on("blur", function()
        {
          ...
          {
            $(this).trigger("focus").trigger("select");
            messageArea.show().addClass("alert alert-danger").text("Please enter a valid email address. This must include at least a Capitalized first name followed by a Capitlalized last name.");
          }
          else
          {
              messageArea.removeAttr("class").hide();
